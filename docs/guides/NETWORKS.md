...
into existence. Consumers read it to derive their defaults instead of each
carrying its own copy of a chain id, a port number and an address prefix.
...
answers, and under what identity. Nothing here is consensus-relevant, and a
change to it never needs a protocol release train.
...
How the file reaches another repository is not yet settled, and no consumer
reads it yet. Nothing outside this repository should hand-copy it in the
meantime — a copied constant is the problem this file exists to remove.
